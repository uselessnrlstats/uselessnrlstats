...
half_difference <- match_results |>
  ...
  mutate(
    ...
    half_score_diff = second_half_score - score_for_ht) |>
...
final_table <- half_difference |>
 ...
 # prependContent(tags$style("table tr:nth-child(3) td {background-color: #ffd6f0;}")) |>
  prependContent(h5(class = "title",
                    ...
                    "<u>FOOT ON THE PEDAL</u><br>NRL/NSWRL teams with the largest difference in a match<br>between points scored in the 2nd half and 1st half" |> HTML()))
final_table

saveWidget(final_table, "tables/html/foot_on_pedal.html")
webshot(url = "tables/html/foot_on_pedal.html", 
        file = "tables/png/foot_on_pedal.png", 
        selector = "div", zoom = 4,
        vwidth = 820)
