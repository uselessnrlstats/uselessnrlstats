...
##### Analysis #####
names_to_filter <- c("Charlie", "Charly", "Charles")
...
name_plot <- ggplot() +
  ...
  theme_classic() +
  geom_text_repel(
    data = name_trends |> filter(year %in% c(1932, 1988, 2014)),
  ...
  labs(
    title = "First Name Frequency: CHARLES",
    subtitle = "The number of players with the first name **Charles**, **Charlie** or **Charly** that have played in each season of the NSWRL/NRL"
  )
name_plot

ggsave(filename = "plots/charles_name_years.png", plot = name_plot, device = "png", type = "cairo",
       width = 10, height = 8, units = "cm")

ggsave(filename = "plots/charles_name_years.pdf", plot = name_plot, 
       width = 6, height = 5, units = "in")
pdf_convert("plots/charles_name_years.pdf", 
            format = "png", dpi = 1000,
            filenames = "plots/charles_name_years.png")
x
