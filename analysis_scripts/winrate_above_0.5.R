##### Description #####
# An R script to look at player win-rates always being above 0.5
...
win_rates <- player_match_data |>
  ...
  filter(player_id %in% unique(player_id)[1:20] | full_name == "Kenny Bromwich") |>
  group_by(player_id) |>
  mutate(label = paste0(ordinal(rank), ": ", full_name, " (", max(match_no), ifelse(last_year == 2024, "*", ""), ")"),
         label = ifelse(last_name == "Bromwich", paste0("<b>", label, "</b>"), label)) |>
  ungroup()
...
win_rate_plot <- ggplot(win_rates) +
  theme_classic() +
  geom_segment(x = 0, y = 0.50, xend = 360, yend = 0.50, col = "red", linetype = 2) +
  geom_line(aes(x = match_no, y = win_rate, colour = full_name), linewidth = 1, alpha = 0.8) +
  scale_colour_manual(values = c('#4477AA', '#66CCEE', '#228833','#CCBB44', '#EE6677', '#AA3377'), #c(brewer.pal(6, "Set2")),
  ...
  scale_x_continuous(limits = c(0,360), expand = c(0.01,0)) +
  ...
  labs(
    x = "Game Number",
    y = "Career Win Rate",
    title = "Most NSWRL/NRL Matches with a career win rate\nalways >50%"
  ) +
  theme(
...
ggsave(filename = "plots/win_rate_above_0.5_plot.pdf", plot = win_rate_plot, device = cairo_pdf,
       width = 5, height = 5, units = "in")
pdf_convert("plots/win_rate_above_0.5_plot.pdf", 
            format = "png", dpi = 1000,
            filenames = "plots/win_rate_above_0.5_plot.png")
