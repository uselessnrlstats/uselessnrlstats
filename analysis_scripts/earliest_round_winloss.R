##### Description #####
# An R script to look at the earliest in a season where all teams have a win and a loss
...
ladder_data |>
   group_by(competition_year, year, round) |>
   summarise(all_wins = all(wins > 0),
             all_losses = all(losses > 0),
             both = all_wins & all_losses,
             .groups = "drop") |>
  filter(both) |>
  group_by(competition_year, year) |>
  filter(round == min(round)) |>
  ungroup() |>
  arrange(round, desc(year))
