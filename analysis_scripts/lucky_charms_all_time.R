...
team_matches <- match_results |>
  ...
  group_by(competition_year, team_unique) |>
  summarise(matches_all = list(match_id),
            .groups = "drop")

player_team_matches <- player_match_data |>
  left_join(team_data |> select(team_name, team_unique), by = c("team" = "team_name")) |>
  select(match_id, team_unique, player_id) |>
  left_join(match_data |> select(match_id, competition_year, round), by = "match_id") |>
  mutate(year = gsub(".+ (\\d+)", "\\1", competition_year) |> as.numeric()) |>
  filter(year >= 1998, grepl("Round", round)) |>
  group_by(competition_year, team_unique, player_id) |>
  summarise(match_in = list(match_id),
            .groups = "drop") |>
  left_join(team_matches, by = c("competition_year", "team_unique")) |>
  rowwise() |>
...
win_rate_change <- player_team_matches |>
  select(competition_year, match_id, team_unique, player_id, in_match) |>
  left_join(match_results |>
              left_join(team_data |> select(team_name, team_unique), by = c("team" = "team_name")) |> 
              select(match_id, team_unique, result), by = c("match_id", "team_unique")) |>
  group_by(competition_year, team_unique, player_id) |>
  summarise(
  ...
  select(player_id, competition_year, team_unique, full_name, matches_in, record_in, win_rate_in, matches_out, record_out, win_rate_out, diff_when_in) |>
  filter(matches_in >= 6 & matches_out >= 6) |>
  arrange(team_unique, desc(diff_when_in), desc(matches_in + matches_out)) |>
...
final_table1 <- lucky_charms |>
  ...
  select(competition_year, team_unique, full_name, record_in, record_out, diff_when_in) |>
  rename(
