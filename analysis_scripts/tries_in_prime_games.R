##### Description #####
# An R script to look at players who scored in prime-numbered games
...
player_match_data |>
  ...
  select(player_id, first_year, career_years, full_name, try_matches) |>
  rowwise() |>
  mutate(n_try_matches = length(try_matches),
         try_prime = list(is_prime(try_matches)),
         only_prime = !any(try_prime),
         games_missed = sum(!try_prime)) |>
  View()
