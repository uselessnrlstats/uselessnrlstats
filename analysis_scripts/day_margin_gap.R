...
##### Analysis #####
top_dates <- match_data |>
  rowwise() |>
  mutate(margin = max(home_team_score, away_team_score) - min(home_team_score, away_team_score)) |>
  group_by(date) |>
  #filter(n() == 2) |>
  summarise(
  ...
  arrange(desc(result_diff)) |>
  filter(
    #result_diff == max(result_diff, na.rm = TRUE),
    row_number() <= 4
  ) |>
  pull(date)

match_margins <- match_data |>
  select(competition_year, round, date, home_team, home_team_score, away_team, away_team_score) |>
  rowwise() |>
  mutate(margin = max(home_team_score, away_team_score) - min(home_team_score, away_team_score)) |>
  ungroup() |>
  filter(date %in% top_dates) |>
  mutate(
  ...
  select(year, round, date, home_team, match_score, away_team, margin)
