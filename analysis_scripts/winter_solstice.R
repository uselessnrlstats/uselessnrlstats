##### Description #####
# An R script to look at winter solstice data
...
match_results <- match_data |>
  ...
  mutate(home_away = toupper(substr(home_away, 1, 1)),
         ...
         result = case_when(
           ...
           .default = "D")) |>
  select(match_id, competition_year, year, round, home_away, team, opposition, score_for, score_against, result) |>
...
solstice_21st <- c(1940, 1944, 1948, 1952, 1956, 1960, 1964, 1968, 1969, 1972, 1973, 1976, 1977, 1980, 1981, 1984, 1985, 1988, 1989, 1992, 1993, 1996, 1997, 2000, 2001, 2002, 2004, 2005, 2006, 2008, 2009, 2010, 2012, 2013, 2014, 2016, 2017, 2018, 2020, 2021, 2022)

solstice_dates <- tibble(year = 1908:2023) |>
  rowwise() |>
  mutate(date = paste(c(year, "6", ifelse(year %in% solstice_21st, 21, 22)), collapse = "/")) |>
  ungroup() |>
  mutate(date = as_date(date, format = "%Y/%m/%d")) 

solstice_matches <- match_data |>
  filter(date %in% solstice_dates$date)

##### Analysis #####
wins_table <- match_results |>
  filter(match_id %in% solstice_matches$match_id) |>
