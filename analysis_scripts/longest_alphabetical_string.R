...
##### Analysis #####
longest_alph_string <- function(string, reverse = FALSE, repeats = TRUE) {
  # Create vector of letters
  name <- str_split(string, pattern = "")[[1]]
  
  alph_cumsum <- if (reverse & repeats) {
    c(TRUE, name[2:length(name)] > name[1:length(name) - 1]) |> cumsum()
  } else if (reverse & !repeats) {
    c(TRUE, name[2:length(name)] >= name[1:length(name) - 1]) |> cumsum()
  } else if (!reverse & repeats) {
    c(TRUE, name[2:length(name)] < name[1:length(name) - 1]) |> cumsum()
  } else if (!reverse & !repeats) {
  ...
  longest_str <- name[which(alph_cumsum == (table(alph_cumsum) |> which.max() |> as.numeric()))]
  
  return(longest_str)
}


alphabetical_strings <- player_data |>
  filter(first_name != "?",
         !is.na(first_name),
         nchar(first_name) > 1) |>
  ...
  mutate(full_name_letters = gsub("[^a-zA-Z]", "", full_name) |>
           str_to_upper()) |>
  rowwise() |>
  mutate(longest_alph_list = list(longest_alph_string(full_name_letters, reverse = FALSE, repeats = TRUE)),
