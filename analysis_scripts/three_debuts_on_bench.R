##### Description #####
# An R script to look at teams with three debutants or more on the bench
...
##### Analysis #####
three_bench_debuts <- player_match_data |>
  ...
  filter(game_number == 1 & position == "B") |>
  group_by(match_id, competition_year, round, date, team, opposition_team) |>
  summarise(
    n_debuts = n(),
    debutants = paste0(paste0(number, ". ", full_name), collapse = "<br>"),
    .groups = "drop"
  ) |>
  mutate(year = gsub(".+ (\\d+)", "\\1", competition_year) |> as.numeric(),
         round = gsub("Round (\\d+)", "Rd\\1", round)) |>
  filter(n_debuts >= 3, year >= 1998) |>
  arrange(date) |>
  select(year, round, team, opposition_team, debutants)
