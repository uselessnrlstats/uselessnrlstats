...
win_rates <- player_match_data |>
  ...
  select(player_id, full_name, team, match_no, win_rate, final_win_rate) |>
  filter(player_id %in% c(476, 10917, 1375, 533, 17283)) |>
  arrange(desc(max(match_no))) |>
  mutate(full_name = factor(full_name, levels = unique(full_name))) |>
  group_by(player_id) |>
  mutate(label = paste0(full_name, " (", max(match_no), ifelse(full_name == "Tyson Frizell", "*", ""), ")")) |>
  ungroup()
...
win_rate_plot <- ggplot(win_rates) +
  theme_classic() +
  geom_line(aes(x = match_no, y = win_rate, colour = full_name), linewidth = 1) +
...
ggsave(filename = "plots/win_rates_plot.pdf", plot = win_rate_plot, device = cairo_pdf,
       width = 5, height = 5, units = "in")
pdf_convert("plots/win_rates_plot.pdf", 
            format = "png", dpi = 1000,
            filenames = "plots/win_rates_plot.png")
