...
##### Analysis #####
player_experience <- player_match_data |>
...
home_players <- player_match_data |>
  ...
  ungroup() |>
  group_by(match_id) |>
  filter(team == unique(team)[1]) |>
  ungroup() |>
  mutate(team = "home_player") |>
  pivot_wider(id_cols = c(match_id, position), 
              names_from = team, values_from = player_id) |>
  left_join(match_data |> select(match_id, home_team),  by = "match_id") |>
  left_join(player_experience, by = c("match_id", "home_player" = "player_id")) |>
  rename(home_experience = experience)

away_players <- player_match_data |>
  select(player_id, match_id, team, position) |>
  group_by(team, position) |>
  mutate(position = case_when(
    row_number() > 1 ~ paste0(position, row_number()),
  ...
  ungroup() |>
  group_by(match_id) |>
  filter(team == unique(team)[2]) |>
  ungroup() |>
  mutate(team = "away_player") |>
  pivot_wider(id_cols = c(match_id, position), 
              names_from = team, values_from = player_id) |>
  left_join(match_data |> select(match_id, away_team),  by = "match_id") |>
  left_join(player_experience, by = c("match_id", "away_player" = "player_id")) |>
  rename(away_experience = experience)

overall_experience <- home_players |>
  full_join(away_players, by = c("match_id", "position")) |>
  filter(!(is.na(home_team) | is.na(away_team))) |>
  group_by(match_id, home_team, away_team) |>
  summarise(
    home_more = sum(home_experience > away_experience),
    away_more = sum(away_experience > home_experience),
    equal = sum(home_experience == away_experience),
    most = max(home_more, away_more),
    .groups = "drop"
  ) |>
