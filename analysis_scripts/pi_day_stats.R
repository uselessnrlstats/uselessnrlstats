...
##### Analysis #####
average_jersey <- player_match_data |>
  group_by(player_id) |>
  summarise(total_matches = n(),
            teams = list(unique(team)),
            number_total = sum(number),
            av_number = mean(number, na.rm = TRUE)) |>
  mutate(av_num_1pi = abs(av_number - pi),
         av_num_2pi = abs(av_number - 2*pi),
         av_num_3pi = abs(av_number - 3*pi),
         av_num_4pi = abs(av_number - 4*pi),
         av_num_5pi = abs(av_number - 5*pi)) |>
  arrange(av_num_1pi) |>
  left_join(player_data |> select(player_id, full_name, birthdate),
            by = "player_id") |>
  left_join(player_career_years |> select(player_id, career_years),
            by = "player_id") |>
  select(player_id, full_name, career_years, teams, total_matches, number_total, av_number, av_num_1pi)

player_number_counts <- player_match_data |>
  count(player_id, number) |>
  group_by(player_id) |>
  summarise(number_summary = paste0(paste0("<b>", number, "</b>"), paste0("(", n, ")"), collapse = ", "))
