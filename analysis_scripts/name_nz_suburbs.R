...
nz_suburbs <- st_read("analysis_scripts/extra_data/NZ_localities/nz-suburbs-and-localities.shp") |>
  st_transform(crs = st_crs(2193)) |>
  rename(SUBURBS = name) |>
  mutate(SUBURBS = SUBURBS |> tolower() |> toTitleCase()) |>
  filter(type %in% c("Locality", "Suburb", "Conservation Land", "Lake", "Inland Bay", "Island"))

##### Analysis #####
# Player Career Summaries
player_teams <- player_match_data |>
  left_join(teams_data, by = c("team" = "team_name")) |>
  group_by(player_id) |>
  summarise(teams = list(unique(team_unique)))

players_in <- player_data |>
  rowwise() |>
  filter(
    any(c(full_name, first_name, last_name) %in% nz_suburbs$SUBURBS)
  ) |>
  left_join(player_teams, by = "player_id") |>
  filter(any(c("New Zealand Warriors") %in% teams))
...
name_map <- ggplot(player_suburbs) +
  ...
  scale_fill_manual(name = "<b>In a Warrior's Name:</b>",
                    labels = c("No", "Yes"),
                    values = c("#262c68", "#008445"),
                    guide = guide_legend(
                      position = "bottom",
  ...
  labs(
    title = "New Zealand suburbs in Warriors player names"
  )
name_map
