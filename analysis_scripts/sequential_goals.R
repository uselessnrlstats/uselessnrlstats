...
##### Analysis #####
most_decreasing_goals <- player_match_data |>
  select(player_id, match_id, team, opposition_team, goals) |>
  ...
  group_by(player_id) |>
  mutate(career_match = row_number(),
         add_one_goal = (goals == (lag(goals) - 1)) |> replace_na(FALSE)) |>
  filter(add_one_goal | lead(add_one_goal)) |>
  mutate(seq_no = cumsum(!add_one_goal)) |>
  ungroup() |>
  group_by(player_id, seq_no) |>
  mutate(seq_length = n()) |>
  ungroup() |>
  arrange(desc(seq_length), player_id, seq_no, date) |>
