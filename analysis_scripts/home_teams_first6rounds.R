...
##### Analysis #####
home_team_wins <- match_results |>
  filter(round <= 6, home_away == "H") |>
  group_by(year) |>
  summarise(n_games = n(),
            n_wins = sum(result == "W"),
            .groups = "drop") |>
  mutate(strike_rate = n_wins / n_games) |>
...
final_table <- home_team_wins |>
  #filter(row_number() <= 10) |>
  mutate(
    strike_rate = percent(strike_rate)
  ) |>
  rename(
    Year = year,
    Matches = n_games,
    `Home Team Wins` = n_wins,
    `Win Rate` = strike_rate
  ) |>
  formattable(
    list(
      Year = bold_formatter,
      `Win Rate` = bold_formatter
    ),
    align = c("r", "r", "c", "l"),
  ...
  prependContent(h4(class = "title", 
                    ...
                    "Home Team Record Rds1-6")) |>
  prependContent(h5(class = "subtitle",
                    style = "font-family: Roboto Regular; margin-left: 25px; display: flex; flex-direction: column",
                    "NRL Era (1998-2024) win rate of home <br> teams in the first six rounds" |> HTML()))
final_table

saveWidget(final_table, "tables/html/home_team_wins6.html")
webshot(url = "tables/html/home_team_wins6.html", 
        file = "tables/png/home_team_wins6.png", 
