##### Description #####
# An R script to look at players whose names spell words with their alternating letters
...
##### Analysis #####
player_data |> filter(first_name != "?",
       !is.na(first_name),
       nchar(first_name) > 1) |>
  left_join(player_career_years |> select(player_id, career_years), by = "player_id") |>
  select(player_id, career_years, full_name) |>
  mutate(full_name_letters = gsub("[^a-zA-Z]", "", full_name) |>
           str_to_upper() |>
           str_split(pattern = "")) |>
  rowwise() |>
  mutate(n_letters = length(full_name_letters),
         odd_letters = (full_name_letters[seq(1, n_letters, by = 2)]) |>
           paste0(collapse = "") |> tolower(),
         even_letters = (full_name_letters[seq(2, n_letters, by = 2)]) |>
           paste0(collapse = "") |> tolower()) |>
  ungroup() |>
  mutate(odd_is_word = (odd_letters %in% dict1),
         even_is_word = (even_letters %in% dict2)) |>
  rowwise() |>
  mutate(word = case_when(
           (odd_is_word & !even_is_word) ~ odd_letters,
           (!odd_is_word & even_is_word) ~ even_letters,
           (odd_is_word & even_is_word) ~ paste(c(odd_letters, even_letters), collapse = " | "),
           .default = NA
         )) |>
  View()
...
final_table <- first_name_initials |>
  ...
  prependContent(h5(class = "title",
                    ...
                    "Most unique first name initials in an NRL/NSWRL Grand Final"))
final_table

saveWidget(final_table, "tables/html/gf_first_name_init.html")
webshot(url = "tables/html/gf_first_name_init.html", 
        file = "tables/png/gf_first_name_init.png", 
        selector = "div", zoom = 4,
        vwidth = 760)
