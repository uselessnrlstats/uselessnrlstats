...
bris_suburbs <- st_read("analysis_scripts/extra_data/BCC_suburbs/Suburb_Boundaries.shp") |>
  st_transform(crs = st_crs(4326)) |>
  mutate(SUBURB_NAM = SUBURB_NAM |> tolower() |> toTitleCase())
...
players_in <- player_data |>
  rowwise() |>
  filter(
    any(c(full_name, first_name, last_name) %in% bris_suburbs$SUBURB_NAM)
  ) |>
  left_join(player_teams, by = "player_id") |>
  filter("Brisbane Broncos" %in% teams)

player_data |>
  rowwise() |>
  filter(
    any(str_detect(string = tolower(first_name), pattern = tolower(bris_suburbs$SUBURB_NAM))) |
      any(str_detect(string = tolower(last_name), pattern = tolower(bris_suburbs$SUBURB_NAM))) |
      any(str_detect(string = tolower(full_name), pattern = tolower(bris_suburbs$SUBURB_NAM)))
  ) |>
  left_join(player_teams, by = "player_id")

player_suburbs <- bris_suburbs |>
  mutate(centroid = st_centroid(geometry)) |>
  rowwise() |>
  mutate(
    players = paste(
      sort(players_in$full_name[c(which(players_in$first_name == SUBURB_NAM), which(players_in$last_name == SUBURB_NAM))]),
      collapse = ","),
    players = gsub(SUBURB_NAM, paste0("underline(bold('", SUBURB_NAM, "'))"), players),
