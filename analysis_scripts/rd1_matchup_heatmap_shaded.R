...
rd1_2025 <- data.frame(
  team1 = c("Brisbane Broncos", "Canberra Raiders", "Canterbury Bankstown Bulldogs", "Cronulla Sutherland Sharks", "Dolphins", "Manly Warringah Sea Eagles", "Melbourne Storm", "New Zealand Warriors", "Newcastle Knights", "North Queensland Cowboys", "Parramatta Eels", "Penrith Panthers", "South Sydney Rabbitohs", "St George Illawarra Dragons", "Sydney Roosters", "Wests Tigers"),
  team2 = c("Sydney Roosters", "New Zealand Warriors", "St George Illawarra Dragons", "Penrith Panthers", "South Sydney Rabbitohs", "North Queensland Cowboys", "Parramatta Eels", "Canberra Raiders", "Wests Tigers", "Manly Warringah Sea Eagles", "Melbourne Storm", "Cronulla Sutherland Sharks", "Dolphins", "Canterbury Bankstown Bulldogs", "Brisbane Broncos", "Newcastle Knights")
)
...
plot_data <- rbind(
  ...
  complete(team1, team2,
           fill = list(count = NA, matches = NA))

##### Plot #####
fm <- 2

rd1_matchups_plot <- ggplot(plot_data) +
  geom_tile(aes(x = team1, y = team2, fill = count), 
            colour = "grey90", linewidth = 0.5, show.legend = TRUE) +
  ...
  geom_text(data = plot_data |> filter(!is.na(count)),
            aes(x = team1, y = team2, label = count), colour = "white", family = "Montserrat") +
  ...
  scale_fill_gradientn(colours = c("#BAE9DE", "#98DECD", "#76D3BD", "#54C9AD", "#43A08A", "#327867", "#215045", "#102822"),
                       na.value = "white",
                       trans = "log",
                       guide = "none") +
  ...
  labs(
    ...
    subtitle = "<span style = 'color: #54C9AD;'>Teal matches</span> have occurred before in a NSWRL/NRL Rd1 (# times shown in white)<br>
    <span style = 'color: #1874CD;'>Blue matches</span> are to be played in Rd1 2025"
  )
#rd1_matchups_plot

ggsave(filename = "plots/rd1_matchups2.pdf", plot = rd1_matchups_plot, device = cairo_pdf,
       width = 9, height = 9, units = "in")
pdf_convert("plots/rd1_matchups2.pdf", 
            format = "png", dpi = 1000,
            filenames = "plots/rd1_matchups2.png")
