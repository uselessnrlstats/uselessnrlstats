...
each_vowel <- player_data |>
  ...
  mutate(n_letters = length(full_name_letters),
         uniq_vowels = list(VOWELS[which(VOWELS %in% full_name_letters)]),
         vowels = list(full_name_letters[which(full_name_letters %in% VOWELS)]),
         n_uniq_vowels = length(uniq_vowels),
         n_vowels = length(vowels)) |>
  filter(n_vowels == 5, n_uniq_vowels == 5) |>
