...
initial_data <- player_data |>
  ...
  mutate(first_name_initial = substr(first_name, 1, 1) |> 
         ...
         last_name_initial = substr(last_name, 1, 1) |> 
           str_to_upper() |>
           factor(levels = LETTERS, ordered = TRUE)) |>
  select(player_id, first_name, last_name, first_name_initial, last_name_initial)
...
initiagami <- ggplot(plot_data) +
  ...
  geom_text(aes(x = last_name_initial, y = first_name_initial,
                    label = n),
                colour = "white", size = 3, alpha = 1, vjust = 0.55, na.rm = TRUE) +
  scale_fill_gradientn(name = "# Players",
                       colours = paletteer_c("grDevices::Magenta", 30) |> rev(),
 ...
 theme(
    ...
    legend.title = element_text(family = "Montserrat", size = 5*fm, hjust = 0.6),
  ...
  labs(
    ...
    title = "Initiagami: NRL/NSWRL"
  )
initiagami

ggsave(filename = "plots/initiagami.pdf", plot = initiagami, 
       device = cairo_pdf, width = 8, height = 8, units = "in")
pdf_convert("plots/initiagami.pdf", 
            format = "png", dpi = 1000,
            filenames = "plots/initiagami.png")
