# Rugbyleagueproject player data scrape to be run weekly
...
url = "https://www.rugbyleagueproject.org/competitions/nrl/seasons.html"
driver.get(url)
...
# Load in used IDs
# Teams
for file_name in [os.path.join(dir_name, 'team_data\\team_data_' + str(x) + '.csv') for x in range(117)][0:117]:
    team_file = pd.read_csv(file_name)
    for x in list(team_file['team_name']):
        used_teams.append(x)
# Players
for file_name in [os.path.join(dir_name, 'player_names_data\\player_names_data_' + str(x) + '.csv') for x in range(117)][0:117]:
    player_file = pd.read_csv(file_name)
    for x in list(player_file['player_id']):
        used_players.append(str(x))
# Coaches
for file_name in [os.path.join(dir_name, 'coach_data\\coach_data_' + str(x) + '.csv') for x in range(117)][0:117]:
    coach_file = pd.read_csv(file_name)
    for x in list(coach_file['coach_id']):
        used_coaches.append(str(x))
# Referees
for file_name in [os.path.join(dir_name, 'ref_data\\ref_data_' + str(x) + '.csv') for x in range(117)][0:117]:
    ref_file = pd.read_csv(file_name)
    for x in list(ref_file['ref_id']):
        used_refs.append(str(x))
# Venues
for file_name in [os.path.join(dir_name, 'venue_data\\venue_data_' + str(x) + '.csv') for x in range(117)][0:117]:
    venue_file = pd.read_csv(file_name)
    for x in list(venue_file['venue_id']):
        used_venues.append(str(x))

# 30087 = Cumberland Trial Match 1908
# "13283", "13284", "13422", "13423". "13562", "13563", "13700", "13701", "13838", "13839"]
matches_to_skip = ["30087"]
...
seasons = driver.find_elements(
    "xpath", "//div[@id='content']/table[@class='list']/tbody/tr/td[1]/a")
# loop through all listed seasons
for season_index in range(117, len(seasons)):
    # Go to season results
    seasons[season_index].click()
    wait.until(EC.element_to_be_clickable(
        (By.XPATH, "//main[@id='page_content']/ul/li[5]/a"))).click()
    # Deal with different super league format
    competition_page = driver.find_element(
        "xpath", "//main[@id='page_content']/h1").text
    if competition_page == "Super League 1997 Telstra Cup":
        competition = "Super League 1997"
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, "//main[@id='page_content']/div[@id='content']/div[@id='submenu2']/ul//li[2]/a"))).click()
    else:
        competition = ""
    season_url = driver.current_url
    # Identify all the matches to click on
    matches = driver.find_elements(
        "xpath", "//main[@id='page_content']/div[@id='content']/table[@class='list']/tbody/tr")
    # Loop through all rows of the matches table
    for match_index in range(len(matches)):
        # Figure out the current match round
        if competition != 'Super League 1997':
        ...
        # and match_round not in ["Major Prelim Semi", "Minor Prelim Semi", "Minor Semi Rep."]:
        if len(match_columns) == 11:
            ...
            match_complete = all([x != " " for x in map(
                match_information.__getitem__, [4, 6])])
            # Store the season (to remove trial matches)
            if competition != "Super League 1997":
                competition = match_information[0]
            else:
                match_round = match_information[0]
            # Proceed if not super league, or if super league round is checked
            if (competition != "Super League 1997") or ((competition == "Super League 1997") and (match_round[0:3] != "WCC")):
                # get match ID
                match_id = match_columns[10].find_element(
                    "xpath", "a").get_attribute('href')[43:]
                if match_id not in matches_to_skip and match_complete:
                    ...
                    # Get match round
                    # match_round = wait.until(EC.presence_of_element_located((By.XPATH, "//div[@id='content']/h2"))).text
                    # Get match teams and score
                    score_body = driver.find_element(
                        "xpath", "//table[@class='program']/tbody[1]/tr[2]")
                    match_home_team = score_body.find_element(
                        "xpath", "th[1]/table/tbody/tr/td[1]").text
                    match_home_team_score = score_body.find_element(
                        "xpath", "th[1]/table/tbody/tr/td[2]").text
                    match_away_team_score = score_body.find_element(
                        "xpath", "th[3]/table/tbody/tr/td[1]").text
                    match_away_team = score_body.find_element(
                        "xpath", "th[3]/table/tbody/tr/td[2]").text
                    ...
                    # get match info
                    match_info = driver.find_elements(
                        "xpath", "//table[@class='program']/tbody[@id='match_info']/tr")
                    for match_info_index in range(len(match_info)):
                        match_info_heading = match_info[match_info_index].find_element(
                            "xpath", "th").text
                        if match_info_heading == "Date":
                            match_date = match_info[match_info_index].find_element(
                                "xpath", "td").text
                        elif match_info_heading == "Kick Off":
                            match_time = match_info[match_info_index].find_element(
                                "xpath", "td").text
                        elif match_info_heading in ["Referee", "Referees"]:
                            match_refs = match_info[match_info_index].find_elements(
                                "xpath", "td/a")
                            match_ref_id = [x.get_attribute(
                                'href')[44:] for x in match_refs]
                            ...
                            for ref_index in range(len(match_ref_id)):
                                ...
                                ref_match_data = pd.concat(
                                    [ref_match_data, entry], axis=0)
                        elif match_info_heading == "Crowd":
                            match_crowd = match_info[match_info_index].find_element(
                                "xpath", "td").text
                        elif match_info_heading == "Venue":
                            match_venue_id = match_info[match_info_index].find_element(
                                "xpath", "td/a").get_attribute('href')[42:]
                            # add venue to database if it isn't there already
                            if match_venue_id not in used_venues:
                                match_info[match_info_index].find_element(
                                    "xpath", "td/a").click()
                                # find key info
                                venue_name = wait.until(EC.presence_of_element_located(
                                    (By.XPATH, "//main[@id='page_content']/h1"))).text
                                venue_page_dts = driver.find_elements(
                                    "xpath", "//div[@id='content']/dl/dt")
                                venue_page_dds = driver.find_elements(
                                    "xpath", "//div[@id='content']/dl/dd")
                                venue_nc_name = venue_page_dds[[
                                    x.text for x in venue_page_dts].index("Non-Commercial Name")].text
                                venue_location = venue_page_dds[[
                                    x.text for x in venue_page_dts].index("Location")].text
                                ...
                                venue_data = pd.concat(
                                    [venue_data, entry], axis=0)
                                used_venues.append(match_venue_id)
                                driver.get(match_url)
                                match_info = driver.find_elements(
                                    "xpath", "//table[@class='program']/tbody[@id='match_info']/tr")
                    # Continue with match data for the current match we've just come back to
                    match_stats = driver.find_elements(
                        "xpath", "//table[@class='program']/tbody[@id='match_stats']/tr")
                    for match_stats_index in range(len(match_stats)):
                        match_stats_heading = match_stats[match_stats_index].find_element(
                            "xpath", "th").text
                        if match_stats_heading == "Halftime Score":
                            match_ht_hs = match_stats[match_stats_index].find_element(
                                "xpath", "td").text
                            match_at_hs = match_stats[match_stats_index].find_element(
                                "xpath", "td[2]").text
                        if match_stats_heading == "Penalties":
                            match_ht_penalties = match_stats[match_stats_index].find_element(
                                "xpath", "td").text
                            match_at_penalties = match_stats[match_stats_index].find_element(
                                "xpath", "td[2]").text
                    ...
                    match_scoresheet = driver.find_elements(
                        "xpath", "//table[@class='program']/tbody[@id='match_scoresheet']/tr")
                    for match_ss_index in range(len(match_scoresheet)):
                        # Check if we have a middle column
                        middle_col = match_scoresheet[match_ss_index].find_elements(
                        ...
                        if len(middle_col):
                            ...
                            player_present = match_scoresheet[match_ss_index].find_elements(
                                "xpath", "td[@class='name left']/a")
                            if len(player_present):
                                player_id = player_present[0].get_attribute('href')[
                                    43:]
                                player_scores = match_scoresheet[match_ss_index].find_elements("xpath", "td[2]")[
                                    0].text
                                if player_scores == '':
                                    player_scores = "1"
                                entry = pd.DataFrame([[match_id, player_id, current_score, player_scores]],
                                                     columns=['match_id', 'player_id', 'score', 'n_scores'])
                                player_scoring_data = pd.concat(
                                    [player_scoring_data, entry], axis=0)
                            ...
                            if len(player_present):
                                player_id = player_present[0].get_attribute('href')[
                                    43:]
                                player_scores = match_scoresheet[match_ss_index].find_elements("xpath", "td[3]")[
                                    0].text
                                if player_scores == '':
                                    player_scores = '1'
                                entry = pd.DataFrame([[match_id, player_id, current_score, player_scores]],
                    ...
                    match_teams = driver.find_elements(
                        "xpath", "//table[@class='program']/tbody[@id='match_teams']/tr")
                    for match_teams_index in range(len(match_teams)):
                        # Check if we have a name in our column
                        td_columns = match_teams[match_teams_index].find_elements(
                            by=By.TAG_NAME, value="td")
                        if [x.text for x in td_columns] not in [[" ", ""], [" ", " ", " "], [" "]]:
                            # If we have a name present then we have a number and a player
                            player_position = match_teams[match_teams_index].find_element(
                                by=By.TAG_NAME, value="th").text
                            ...
                            if current_position == "HC":
                                ...
                                if td_columns[0].text != " ":
                                    coach_id = td_columns[0].find_element(
                                        "xpath", "a").get_attribute('href')[43:]
                                    ...
                                    if coach_id not in used_coaches:
                                        ...
                                        coach_data = pd.concat(
                                            [coach_data, entry], axis=0)
                                    ...
                                    coach_match_data = pd.concat(
                                        [coach_match_data, entry], axis=0)
                                # and do the same for coach of club 2
                                if td_columns[3].text != " ":
                                    coach_id = td_columns[3].find_element(
                                        "xpath", "a").get_attribute('href')[43:]
                                    # check if we've stored the coach's name
                                    if coach_id not in used_coaches:
                            ...
                            # If its not the coach then we need to store info for the one (or two) players there
                                ...
                                if td_columns[0].text != " ":
                                    player_id = td_columns[0].find_element(
                                        "xpath", "a").get_attribute('href')[43:]
                                    ...
                                    if player_id not in used_players:
                                        ...
                                        player_name1 = td_columns[0].find_element(
                                            "xpath", "a").text
                                        td_columns[0].find_element(
                                            "xpath", "a").click()
                                        # Find titlecase name
                                        player_name2 = driver.find_element(
                                            "xpath", "//main[@id='page_content']/h1").text
                                        # set birthday and birthplace to blank
                                        player_birthday = player_birthplace = ""
                                        # Find birthday and birthplace
                                        vital_statistics = driver.find_elements(
                                            "xpath", "//div[@class='stats']/h3[1]")
                                        if len(vital_statistics) == 1:
                                            if vital_statistics[0].text == "Vital Statistics":
                                                player_vs_dts = [x.text for x in driver.find_elements(
                                                    "xpath", "//div[@class='stats']/dl/dt")]
                                                player_vs_dds = [x.text for x in driver.find_elements(
                                                    "xpath", "//div[@class='stats']/dl[1]/dd")]
                                        ...
                                        player_names_data = pd.concat(
                                            [player_names_data, entry], axis=0)
                                    ...
                                    player_match_data = pd.concat(
                                        [player_match_data, entry], axis=0)
                                # check if club 2 has a player in that position
                                if td_columns[3].text != " ":
                                    player_id = td_columns[3].find_element(
                                        "xpath", "a").get_attribute('href')[43:]
                                    # check if we've stored the player's name. If not, collect their data.
                                    if player_id not in used_players:
                                        # collect all player data and then
                                        player_name1 = td_columns[3].find_element(
                                            "xpath", "a").text
                                        td_columns[3].find_element(
                                            "xpath", "a").click()
                                        # Find titlecase name
                                        player_name2 = driver.find_element(
                                            "xpath", "//main[@id='page_content']/h1").text
    ...
    match_data.to_csv(os.path.join(
        dir_name, r'match_data\\match_data_' + str(season_index) + r'.csv'))
    team_data.to_csv(os.path.join(
        dir_name, r'team_data\\team_data_' + str(season_index) + r'.csv'))
    player_names_data.to_csv(os.path.join(
        dir_name, r'player_names_data\\player_names_data_' + str(season_index) + r'.csv'))
    coach_data.to_csv(os.path.join(
        dir_name, r'coach_data\\coach_data_' + str(season_index) + r'.csv'))
    ref_data.to_csv(os.path.join(
        dir_name, r'ref_data\\ref_data_' + str(season_index) + r'.csv'))
    venue_data.to_csv(os.path.join(
        dir_name, r'venue_data\\venue_data_' + str(season_index) + r'.csv'))
    player_match_data.to_csv(os.path.join(
        dir_name, r'player_match_data\\player_match_data_' + str(season_index) + r'.csv'))
    player_scoring_data.to_csv(os.path.join(
        dir_name, r'player_scoring_data\\player_scoring_data_' + str(season_index) + r'.csv'))
    coach_match_data.to_csv(os.path.join(
        dir_name, r'coach_match_data\\coach_match_data_' + str(season_index) + r'.csv'))
    ref_match_data.to_csv(os.path.join(
        dir_name, r'ref_match_data\\ref_match_data_' + str(season_index) + r'.csv'))
    # Reset dataframes
    match_data = pd.DataFrame(columns=['competition', 'round', 'date', 'time', 'match_id', 'venue_id', 'crowd', 'home_team', 'home_team_ht_score',
                              'home_team_score', 'home_team_penalties', 'away_team', 'away_team_ht_score', 'away_team_score', 'away_team_penalties'])
    team_data = pd.DataFrame(columns=['team_short', 'team_name'])
    player_names_data = pd.DataFrame(
        columns=['player_id', 'player_name1', 'player_name2', 'birthday', 'birthplace'])
    coach_data = pd.DataFrame(columns=['coach_id', 'coach_name'])
    ref_data = pd.DataFrame(columns=['ref_id', 'ref_name'])
    venue_data = pd.DataFrame(
        columns=['venue_id', 'venue_name', 'non-commercial_name', 'location'])
    player_match_data = pd.DataFrame(columns=[
                                     'player_id', 'match_id', 'team', 'opposition_team', 'number', 'position', 'captain'])
    player_scoring_data = pd.DataFrame(
        columns=['match_id', 'player_id', 'score', 'n_scores'])
...
players_url = "https://www.rugbyleagueproject.org/competitions/nrl/players.html"
driver = webdriver.Chrome(service=s)
driver.get(players_url)
player_rows = driver.find_elements(
    "xpath", "//div[@id='content']/table[@class='list x']/tbody/tr")
driver.maximize_window()

for player_index in range(len(player_rows)):
    player_columns = player_rows[player_index].find_elements("xpath", "td")
    player_id = player_columns[0].find_element(
        "xpath", "a").get_attribute('href')[43:]
