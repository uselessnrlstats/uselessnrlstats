...
# Load in used IDs
# Teams
for file_name in [os.path.join(dir_name, 'team_data\\team_data_' + str(x) + '.csv') for x in range(118)][0:118]:
    team_file = pd.read_csv(file_name)
    for x in list(team_file['team_name']):
        used_teams.append(x)
# Players
for file_name in [os.path.join(dir_name, 'player_names_data\\player_names_data_' + str(x) + '.csv') for x in range(118)][0:118]:
    player_file = pd.read_csv(file_name)
    for x in list(player_file['player_id']):
        used_players.append(str(x))
# Coaches
for file_name in [os.path.join(dir_name, 'coach_data\\coach_data_' + str(x) + '.csv') for x in range(118)][0:118]:
    coach_file = pd.read_csv(file_name)
    for x in list(coach_file['coach_id']):
        used_coaches.append(str(x))
# Referees
for file_name in [os.path.join(dir_name, 'ref_data\\ref_data_' + str(x) + '.csv') for x in range(118)][0:118]:
    ref_file = pd.read_csv(file_name)
    for x in list(ref_file['ref_id']):
        used_refs.append(str(x))
# Venues
for file_name in [os.path.join(dir_name, 'venue_data\\venue_data_' + str(x) + '.csv') for x in range(118)][0:118]:
...
for season_index in range(118, len(seasons)):
    ...
    if competition_page == "Super League 1997 Telstra Cup":
        ...
        wait.until(EC.element_to_be_clickable((By.XPATH, "//main[@id='page_content']/div[@id='content']/div[@id='submenu2']/ul//li[2]/a"))).click()
    else:
        competition = ""
    ...
    for match_index in range(len(matches)):
        ...
        # Only consider it a match if the row has 11 columns (or 10 for Super League)
        match_columns = matches[match_index].find_elements("xpath", "td")
        # and add an extra empty placeholder to the start of the super league entry
        if competition == "Super League 1997":
            match_columns.insert(0, 0)
        if len(match_columns) == 11:
            ...
            match_information = [x.text if x != 0 else 0 for x in match_columns]
            # the match is complete if we have a score
            match_complete = all([x != " " for x in map(match_information.__getitem__, [4,6])])
            # Handle match information depending on whether it is super league data:
            if competition != "Super League 1997":
