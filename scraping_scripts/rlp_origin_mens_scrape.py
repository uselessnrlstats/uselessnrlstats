# Rugbyleagueproject origin player data scrape
...
url = "https://www.rugbyleagueproject.org/competitions/state-of-origin/resultsbrief.html"
driver.get(url)
driver.maximize_window()
wait = WebDriverWait(driver, 10)

match_data = pd.DataFrame(columns = ['year',
                                     'date',
                                     'time',
                                     'match_id',
                                     'venue_id',
...
for match_index in range(len(matches)):
    ...
    if len(match_columns) == 12: # and match_round not in ["Major Prelim Semi", "Minor Prelim Semi", "Minor Semi Rep."]:
        ...
        match_year = match_information[0]
        # the match is complete if we have a score
        match_complete = all([x != " " for x in map(match_information.__getitem__, [3,7])])        
        # Proceed if not super league, or if super league round is checked
        # get match ID
        match_id = match_columns[11].find_element("xpath", "a").get_attribute('href')[43:]
        if match_complete:
            match_columns[11].find_element("xpath", "a").click() 
            ...
            if match_home_team not in used_teams:
                entry = pd.DataFrame([[match_information[2], match_home_team]],
            ...
            if match_away_team not in used_teams:
                entry = pd.DataFrame([[match_information[6], match_away_team]],
                                     columns = ['team_short', 'team_name'])
                team_data = pd.concat([team_data, entry], axis = 0)
            ...
            for match_info_index in range(len(match_info)):
                ...
                if match_info_heading == "Date":
                    ...
                    if match_venue_id not in used_venues:
                        ...
                        match_info = driver.find_elements("xpath", "//table[@class='program']/tbody[@id='match_info']/tr")
                elif match_info_heading == "Official Player of the Match":
                    match_pom_id = match_info[match_info_index].find_element("xpath", "td/a").get_attribute('href')[43:]
            ...
            entry = pd.DataFrame([[match_year, match_date, match_time, match_id, match_venue_id, match_crowd, match_pom_id, match_home_team, match_ht_hs, match_home_team_score, match_ht_penalties, match_away_team, match_at_hs, match_away_team_score, match_at_penalties]],
                                 columns = ['year', 'date', 'time', 'match_id', 'venue_id', 'crowd', 'pom_id', 'home_team', 'home_team_ht_score', 'home_team_score', 'home_team_penalties', 'away_team', 'away_team_ht_score', 'away_team_score', 'away_team_penalties'])
                ...
                if [x.text for x in td_columns] not in [[" ", ""], [" ", " ", " "], [" "]]:
                    ...
                    # If its not the coach then we need to store info for the one (or two) players there
                        ...
                        if td_columns[0].text != " ":
                            ...
                            player_club_holder = td_columns[0].find_elements("xpath", "div/span/abbr")
                            if player_club_holder != []:
                                player_club = player_club_holder[0].get_attribute("title")
                            else:
                                player_club = ""
                            ...
                            entry = pd.DataFrame([[player_id, match_id, match_home_team, player_club, player_number, current_position, player_captain]],
                                                 columns = ['player_id', 'match_id', 'team', 'club', 'number', 'position', 'captain'])
                        ...
                        if td_columns[3].text != " ":
                            ...
                            player_club_holder = td_columns[3].find_elements("xpath", "div/span/abbr")
                            if player_club_holder != []:
                                player_club = player_club_holder[0].get_attribute("title")
                            else:
                                player_club = ""
                            ...
                            entry = pd.DataFrame([[player_id, match_id, match_away_team, player_club, player_number, current_position, player_captain]],
                                                 columns = ['player_id', 'match_id', 'team', 'club', 'number', 'position', 'captain'])
                            player_match_data = pd.concat([player_match_data, entry], axis = 0)               
...
match_data.to_csv(os.path.join(dir_name, r'match_data_origin.csv'))
team_data.to_csv(os.path.join(dir_name, r'team_data_origin.csv'))
player_names_data.to_csv(os.path.join(dir_name, r'player_names_data_origin.csv'))
coach_data.to_csv(os.path.join(dir_name, r'coach_data_origin.csv'))
ref_data.to_csv(os.path.join(dir_name, r'ref_data_origin.csv'))
venue_data.to_csv(os.path.join(dir_name, r'venue_data_origin.csv'))
player_match_data.to_csv(os.path.join(dir_name, r'player_match_data_origin.csv'))
player_scoring_data.to_csv(os.path.join(dir_name, r'player_scoring_data_origin.csv'))
coach_match_data.to_csv(os.path.join(dir_name, r'coach_match_data_origin.csv'))
ref_match_data.to_csv(os.path.join(dir_name, r'ref_match_data_origin.csv'))
...
players_url = "https://www.rugbyleagueproject.org/competitions/state-of-origin/players.html"
driver = webdriver.Chrome(service = s)
...
refs_url = "https://www.rugbyleagueproject.org/competitions/state-of-origin/referees.html"
driver = webdriver.Chrome(service = s)
driver.get(refs_url)
