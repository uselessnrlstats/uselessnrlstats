...
# Load in used IDs
# Teams
for file_name in [os.path.join(dir_name, 'team_data\\team_data_' + str(x) + '.csv') for x in range(6)][0:6]:
    team_file = pd.read_csv(file_name)
    for x in list(team_file['team_name']):
        used_teams.append(x)
# Players
for file_name in [os.path.join(dir_name, 'player_names_data\\player_names_data_' + str(x) + '.csv') for x in range(6)][0:6]:
    player_file = pd.read_csv(file_name)
    for x in list(player_file['player_id']):
        used_players.append(str(x))
# Coaches
for file_name in [os.path.join(dir_name, 'coach_data\\coach_data_' + str(x) + '.csv') for x in range(6)][0:6]:
    coach_file = pd.read_csv(file_name)
    for x in list(coach_file['coach_id']):
        used_coaches.append(str(x))
# Referees
for file_name in [os.path.join(dir_name, 'ref_data\\ref_data_' + str(x) + '.csv') for x in range(6)][0:6]:
    ref_file = pd.read_csv(file_name)
    for x in list(ref_file['ref_id']):
        used_refs.append(str(x))
# Venues
for file_name in [os.path.join(dir_name, 'venue_data\\venue_data_' + str(x) + '.csv') for x in range(6)][0:6]:
